...
const meta: Meta<typeof BoxPlotChart> = {
    ...
    title: 'reviz-charts/charts/BoxPlot',
    argTypes: {
        orientation: {
            control: {
                type: 'select',
...
// const DATAPOINTS = range(4).map(() => range(12).map(() => randomUniform(1, 10)()).map(v => ({value: v})));
const DATAPOINTS = range(4).map(() => ({values: range(12).map(() => randomUniform(1, 10)())}));
